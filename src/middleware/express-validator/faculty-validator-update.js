...
//   ];
  ...
  const isEitherFacultyOrExternalFacultyProvided = (value, { req }) => {
    console.log('req.body in case of validation', req.body)
    ...
    const externalFacultyUpdate = JSON.parse(req.body.externalFacultyUpdate || '[]');

    if (facultyContainer.length === 0 && externalFacultyDetails.length === 0 && externalFacultyUpdate.length == 0) {
...
// validate org insert
module.exports.updateValidateFaculty = [


    body()
    .custom(isEitherFacultyOrExternalFacultyProvided)
    .withMessage('Details of Other Inventors should not be empty choose atleast one'),
