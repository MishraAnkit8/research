...
module.exports.insertBookChapter = async (bookChapter, files, userName) => {
  ...
  const bookChapterInsertedData = await bookChapterModels.insertBookChapterData(
    bookChapter,
...
module.exports.updatedBookChapter = async (
  ...
  const updateBookChapterDataFiles = files
  ...
  console.log(
    "updatedBookChapterData in service ====>>>",
...
module.exports.deleteBookChapterPublication = async (bookChapterId) => {
  ...
  const bookChapterPublication = await bookChapterModels.deleteBookChapter(
    bookChapterId
  );
...
module.exports.viewBookChapterData = async (bookChapterId, userName) => {
  ...
  const viewBookChapterPublication =
    await bookChapterModels.viewBookChapterData(bookChapterId, userName);
  console.log(
    "viewBookChapterPublication ==>>",
    viewBookChapterPublication.rows[0]
  );
  if (
    viewBookChapterPublication.rows[0] &&
    viewBookChapterPublication.rowCount === 1
  ) {
