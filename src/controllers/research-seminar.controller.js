...
module.exports.renderResearchSeminar = async (req, res, next) => {
  ...
  const statusCode =
    seminarList.status === "Done" ? 200 : seminarList.errorCode ? 400 : 500;
...
module.exports.createResearchSeminar = async (req, res, next) => {
  ...
  const researchSeminarData =
    await researchSeminarServices.insertResearchSeminar(
      req.body,
      req.files,
      userName
    );
  ...
  const statusCode =
    researchSeminarData.status === "Done"
      ? 200
      : researchSeminarData.errorCode
      ? 400
      : 500;
  ...
  res.status(statusCode).send({
    ...
    errorCode: researchSeminarData.errorCode
      ? researchSeminarData.errorCode
      : null,
  });
};
...
module.exports.delResearchSeminar = async (req, res, next) => {
  ...
  const delSeminarData = await researchSeminarServices.deleteResearchSeminar(
    seminarId,
    userName
  );
...
module.exports.updateResearchSeminar = async (req, res, next) => {
  ...
  const updatedSeminar = await researchSeminarServices.updateResearchSeminar(
    seminarId,
    req.body,
    req.files,
    userName
  ...
  const statusCode =
    updatedSeminar.status === "Done"
      ? 200
      : updatedSeminar.errorCode
      ? 400
      : 500;
...
module.exports.viewResearchSeminar = async (req, res, next) => {
  ...
  const viewSeminarDetails = await researchSeminarServices.viewResearchSeminar(
    { seminarId },
    userName
  );
  ...
  const statusCode =
    viewSeminarDetails.status === "Done"
      ? 200
      : viewSeminarDetails.errorCode
      ? 400
      : 500;
  res.status(statusCode).send({
    ...
    errorCode: viewSeminarDetails.errorCode
      ? viewSeminarDetails.errorCode
      : null,
  });
